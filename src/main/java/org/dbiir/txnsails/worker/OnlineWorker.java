...
public class OnlineWorker {
     ...
     * args[2:]: the params for
     */
    public String execute(String[] args) throws SQLException {
        if (args.length < 2)
            return "";
        StringBuffer results = new StringBuffer();
        TemplateSQL templateSQL = this.templates.get(args[0]).getSQLTemplateByIndex(Integer.parseInt(args[1]));

        // record the sql that need validate
        if (templateSQL.isNeedRewriteUnderRC() || templateSQL.isNeedRewriteUnderSI() || shouldSample) {
            ValidationMeta meta = templateSQL.isNeedRewriteUnderRC() ? validationMetaUnderRC[validationMetaIdxUnderRC] :
                    templateSQL.isNeedRewriteUnderSI() ? validationMetaUnderSI[validationMetaIdxUnderSI] : sampleMeta;
            ...
            if (templateSQL.getUniqueKeyNumber() <= args.length - 2) {
                ...
                if (shouldSample) {
                    addSampleMeta(templateSQL.getOp(),
        ...
        try (PreparedStatement stmtc = this.getPreparedStatement(conn, new SQLStmt(rewrite_sql), Arrays.copyOfRange(args, 1, args.length))) {
    ...
    private void validate() throws SQLException {
         ...
         * 2. check the version
         */
        while (Adapter.getInstance().isInSwitchPhase() && !Adapter.getInstance().isAllWorkersReadyForSwitch()) {
        ...
        if (lockManner == CCType.RC_TAILOR) {
            for (int i = 0; i < validationMetaIdxUnderRC; i++) {
                ...
                LockTable.getInstance().tryValidationLock(templateSQL.getRelation(), this.transactionId,
                        validationMeta.getIdForValidation(), lockType, Adapter.getInstance().getCCType());
            }
    ...
    private void validateSingleMeta(ValidationMeta meta) throws SQLException {
        long v = LockTable.getInstance().getHotspotVersion(meta.getTemplateSQL().getRelation(), (long) meta.getIdForValidation());
        if (v >= 0) {
            if (v != meta.getOldVersions()) {
                releaseValidationLocks();
                String msg = String.format("Validation failed for key #%d, %s", meta.getIdForValidation(), meta.getTemplateSQL().getRelation());
                throw new SQLException(msg, "500");
            }
        } else {
            try {
                v = LockTable.getInstance().fetchUnknownVersionCache(meta.getTemplateSQL().getRelation(), meta.getIdForValidation());
                if (v != meta.getOldVersions()) {
                    releaseValidationLocks();
                    String msg = String.format("Validation failed for ycsb_key #%d, usertable", meta.getIdForValidation());
                    throw new SQLException(msg, "500");
                }
            } catch (SQLException ex) {
                releaseValidationLocks();
    ...
    private void releaseValidationLocks() {
        for (int i = this.validationMetaIdx - 1; i >= 0; i--) {
            ...
            LockType lockType = templateSQL.getOp() == 0 ? LockType.SH : LockType.EX;
            LockTable.getInstance().releaseValidationLock(templateSQL.getRelation(), meta.getIdForValidation(), lockType);
            if (lockType == LockType.EX) {
                // update the hot version cache (HVC) if the entry is cached in memory
                LockTable.getInstance().updateHotspotVersion(templateSQL.getRelation(), meta.getIdForValidation(), meta.getOldVersions());
            }
        }
        validationMetaIdx = 0;
    }
    ...
    private boolean shouldRewrite(TemplateSQL templateSQL, CCType t) {
        if ((t == CCType.SI_TAILOR || t == CCType.SER_TRANSITION) && templateSQL.isNeedRewriteUnderSI()) {
            return true;
        }
    ...
    // mode: Connection.TRANSACTION_READ_COMMITTED, TRANSACTION_REPEATABLE_READ, TRANSACTION_SERIALIZABLE
    private void switchConnectionIsolationMode() throws SQLException {
        ...
        if (Adapter.getInstance().isInSwitchPhase() && !switchFinish) {
            ...
            System.out.println(Thread.currentThread().getName() + "switch finish: " + Adapter.getInstance().getNextCCType());
        }
    }
