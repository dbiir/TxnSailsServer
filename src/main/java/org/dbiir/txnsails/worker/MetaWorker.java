...
public class MetaWorker {
    ...
    synchronized public void registerTemplateName(String template_name) {
        templates.computeIfAbsent(template_name, k -> new TransactionTemplate(template_name, globalTemplateTypeGenerator));
        globalTemplateTypeGenerator <<= 1;
    }
    ...
    public void analysisWorkload() {
        ...
        Set<StaticDependencyCycle> cyclesWithTwoConsecutiveReadWrite = findCyclesWithTwoConsecutiveReadWrite(allCycles);
        ...
        for (StaticDependencyCycle cycle: cyclesWithTwoConsecutiveReadWrite) {
            ...
            for (int i = 0; i < edges.size(); i++) {
                if (edges.get(i).getType() == DependencyType.READ_WRITE && edges.get((i + 1) % edges.size()).getType() == DependencyType.READ_WRITE) {
                    edges.get((i + 1) % edges.size()).getFrom().setSQLRewriteByIndex(edges.get((i + 1) % edges.size()).getIdxInFrom(), "SI");
                    edges.get((i + 1) % edges.size()).getTo().setSQLRewriteByIndex(edges.get((i + 1) % edges.size()).getIdxInTo(), "SI");
                }
            }
        }
    ...
    private Set<StaticDependencyCycle> findCyclesWithTwoConsecutiveReadWrite(Set<StaticDependencyCycle> allCycles) {
        Set<StaticDependencyCycle> result = new HashSet<>();
