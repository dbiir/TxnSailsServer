...
public class LockTable {
  ...
  public int fetchUnknownVersionCache(String tableName, int id) throws SQLException {
    ...
    if (workload.equals("ycsb")) {
      PreparedStatement getUserTable =
          connections.get(id % LOAD_THREAD).prepareStatement(GetUserTable.getSQL());
      if (!validationLocks.get("usertable")[id].isEmpty()) {
        // `if` condition is always true, because txnsails should acquire validation lock before
        // they check version
      ...
      PreparedStatement getSavings =
          connections.get(id % LOAD_THREAD).prepareStatement(GetSavingsBalance.getSQL());
      PreparedStatement getChecking =
          connections.get(id % LOAD_THREAD).prepareStatement(GetCheckingBalance.getSQL());
  ...
  private boolean checkAndTryLock(String table, long tid, long key, LockType type)
      throws SQLException {
    int bucketNum = (int) (key % HASH_SIZE);
  ...
  public void tryLock(String table, long key, long tid, LockType type) throws SQLException {
    ...
    if (checkAndTryLock(table, tid, key, type)) return;
    try {
