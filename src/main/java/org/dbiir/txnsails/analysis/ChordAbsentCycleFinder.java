...
//            if (!visited.contains(node)) {
            visited = new HashSet<>();
                dfs_verbose(node, visited, stack, new ArrayList<>(), cycles, new HashSet<>(), new ArrayList<>());
//            }
        }
    ...
    private void dfs_verbose(TransactionTemplate node, Set<TransactionTemplate> visited, Set<TransactionTemplate> stack,
                     List<TransactionTemplate> path, Set<List<String>> cycles,
                     Set<String> edgeNames, List<String> pathDetails) {
        visited.add(node);
        stack.add(node);
        path.add(node);
...
//            pathDetails.add(node.getName() + " --(" + edge.getType() + "," + edge.getEdgeName() + ")--> " + neighbor.getName());
            ...
            if (!visited.contains(neighbor)) {
                ...
                path.add(neighbor);
//                List<TransactionTemplate> cycle = new ArrayList<>(path.subList(index, path.size()));
    ...
    private void dfs(TransactionTemplate node, Set<TransactionTemplate> visited, Set<TransactionTemplate> stack,
                             List<TransactionTemplate> path, List<StaticDependencyGraphEdge> edges, Set<StaticDependencyCycle> cycles,
...
//                    System.out.println("Chord found: " + edge.getFrom().getName() + " -> " + edge.getTo().getName() + " (" + edge.getEdgeName() + ")");
                    return false;
                }
            }
        }
